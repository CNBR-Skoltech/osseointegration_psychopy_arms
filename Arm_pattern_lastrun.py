...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='D:\\Education\\Skoltech\\CNBR\\Остеоинтеграция\\Experiments\\Arm_pattern_lastrun.py',
...
win = visual.Window(
    ...
    monitor='2022_28_10 14:20', color=[0,0,0], colorSpace='rgb',
...
for thisTrial in trials:
    ...
    continueRoutine = True
    # update component parameters for each repeat
    Movement = visual.MovieStim3(
        win=win, name='Movement',
        noAudio = False,
        filename=video,
    ...
    # keep track of which components have finished
    MoveComponents = [Forward, Backward, Neut, Movement]
    ...
    while continueRoutine:
        ...
        # *Forward* updates
        if Forward.status == NOT_STARTED and tThisFlip >= 4.0-frameTolerance:
            # keep track of start time/frame for later
        ...
        # *Backward* updates
        if Backward.status == NOT_STARTED and tThisFlip >= pause + 6-frameTolerance:
            # keep track of start time/frame for later
        ...
        if Backward.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > Backward.tStartRefresh + backward-frameTolerance:
                # keep track of stop time/frame for later
        ...
        # *Neut* updates
        if Neut.status == NOT_STARTED and tThisFlip >= 6-frameTolerance:
            # keep track of start time/frame for later
        ...
        if Neut.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > Neut.tStartRefresh + pause-frameTolerance:
                # keep track of stop time/frame for later
        ...
        if Movement.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > Movement.tStartRefresh + 4.0-frameTolerance:
                # keep track of stop time/frame for later
